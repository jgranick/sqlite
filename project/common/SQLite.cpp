...
static void sqlite_finalize_result( result *r, int exc ) {
	r->first = 0;
	r->done = 1;
	if( r->ncols == 0 )
...
**/
static value sqlite_close( value v ) {
...
**/
static value sqlite_last_insert_id( value db ) {
...
**/
static value sqlite_request( value v, value sql ) {
	database *db;
	result *r;
	const char *tl;
	int i,j;
...
**/
static value sqlite_result_get_length( value v ) {
	result *r;
...
**/
static value sqlite_result_get( value v, value n ) {
	result *r;
	val_check_kind(v,k_result);
	r = val_result(v);
...
**/
static value sqlite_result_get_int( value v, value n ) {
	result *r;
	val_check_kind(v,k_result);
	r = val_result(v);
	if( val_int(n) < 0 || val_int(n) >= r->ncols )
...
**/
static value sqlite_result_get_float( value v, value n ) {
	result *r;
	val_check_kind(v,k_result);
	r = val_result(v);
	if( val_int(n) < 0 || val_int(n) >= r->ncols )
...
DEFINE_PRIM(sqlite_close,1);
DEFINE_PRIM(sqlite_request,2);
DEFINE_PRIM(sqlite_last_insert_id,1);

DEFINE_PRIM(sqlite_result_get_length,1);
DEFINE_PRIM(sqlite_result_get_nfields,1);

DEFINE_PRIM(sqlite_result_next,1);
DEFINE_PRIM(sqlite_result_get,2);
